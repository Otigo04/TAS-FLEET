  ...
  return (
    <section className={`animate-fade-up-delay grid gap-4 sm:gap-6 ${canManage ? 'xl:grid-cols-[360px_1fr]' : ''}`}>
      ...
      <Card className="surface-card">
        ...
        <CardContent>
          <div className="mb-4 grid gap-3 rounded-lg border border-slate-200/80 dark:border-slate-700/60 bg-white/70 dark:bg-slate-900/90 p-3 md:grid-cols-2">
            <Input
          ...
          <div className="space-y-4">
            {filteredVehicles.length === 0 ? (
              <p className="text-sm text-slate-500 dark:text-slate-400">Noch keine Fahrzeuge vorhanden.</p>
            ) : (
              filteredVehicles.map((vehicle) => {
                ...
                return (
                  <div
                    key={vehicle.id}
                    className="rounded-lg border border-slate-200 dark:border-slate-700/60 bg-white dark:bg-slate-900 p-4"
                  >
                    {isEditing ? (
                      <div className="grid gap-3 md:grid-cols-2">
                        ...
                        <div className="flex items-center gap-6">
                          ...
                          <span className="text-xs text-slate-500 dark:text-slate-400">Fahrzeugbild</span>
                        </div>
                        <select
                          className="flex h-10 w-full rounded-md border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 px-3 py-2 text-sm text-slate-900 dark:text-slate-100"
                          value={editStatus}
                      ...
                      <div className="flex flex-col justify-between gap-3 md:flex-row md:items-center">
                        <div className="flex items-center gap-4">
                          {vehicle.avatar_url ? (
                            ...
                            <img src={vehicle.avatar_url} alt="" className="h-10 w-10 rounded-full object-cover bg-slate-100 dark:bg-slate-800 ring-1 ring-slate-200 dark:ring-slate-700" />
                          ) : (
                            <div className="flex h-10 w-10 items-center justify-center rounded-full bg-slate-50 dark:bg-slate-800/50 text-slate-400 ring-1 ring-slate-200 dark:ring-slate-700">
                              <Car className="h-5 w-5" />
                            </div>
                          )}
                          <div>
                            <p className="font-semibold text-slate-900 dark:text-slate-100">{vehicle.license_plate}</p>
